...
vals = {i: sum((1 if x == i else 0) for x in in_nums) for i in range(9)}
...
for day in range(days):
    add_fs = 0
    for bucket, count in list(vals.items()):
        new_bucket = bucket - 1
        if new_bucket < 0:
            new_bucket = 6
            add_fs += count
        vals[bucket] -= count
        vals[new_bucket] += count
    vals[8] += add_fs
    print('Day', day+1, ':', vals)

print(sum(x for x in vals.values()))
